...
- **Z** = 冲针,三层可调(⑤ 里选,默认 `10` 工作高度 / `4` 安全高度 / `0` 移动高度):
  冲孔压到工作高度,打孔阶段所有移动在安全高度,完全不冲孔时(首尾 + 尾部外送带)走 G00 到移动高度。
  **单次冲孔行程 = 工作高度 − 安全高度**,越小节拍越快(当前工程用 `10/6/0` ⇒ 行程 4mm)。
...
市面上常见的"30 音纸带"是给**手工打孔**用的——带面上印着音列网格,靠人眼对齐格子来定位:

![带网格的普通纸带(为手工打孔设计)](docs/images/tape-standard.png)

但本项目的孔位**完全由机器按坐标冲出来**(X 由 `X(col)=6.25+col×(57.5/29)` 算出、Y 由时间步推进),
**不需要靠网格定位**。所以这类网格纸带对本机是**纯浪费**——它更贵,而且为了印格子往往做得很短。
直接买**便宜的空白纸带/纸卷**即可,带面干净反而更好观察孔位对不对。

### 为什么纸带要买长的

纸带消耗量看起来只跟曲目时长有关,但**实际会因 BPM(速度)被放大** —— 原因在八音盒的物理极限:

```
每步时长  step_seconds = 60 / (BPM × 每拍格数)
每步孔距  mm_per_step  = step_seconds × feed_mm_s(默认 16 mm/s)
纸带长度  = 行数 × mm_per_step            (行数 ≈ 时长 ÷ step_seconds)
```

**BPM 越高,`mm_per_step` 越小、孔距越密**。密到音梳来不及复位(来不及重新拨响)时,
就得把 **Y 轴按比例拉伸**(例如 ×1.5),让孔距重新拉够;但拉伸后孔更疏、按原速放会变慢,
于是**再把八音盒本身的走带速度调快同样的比例**,声音时长才不变。

⇒ 纸带长度 = `时长 × feed_mm_s × Y 拉伸系数(k)`。k=1(常规速度,孔距本来就够)时最省料:

| 曲目时长 | 需要纸带(k=1) | 若 Y 拉伸 150%(k=1.5) |
|---|---|---|
| 30 秒 | 0.48 m | 0.72 m |
| **1 分钟** | **0.96 m** | **1.44 m** |
| 2 分钟 | 1.92 m | 2.88 m |
...
**实测实例**(本机打的一首 Marshmello《Alone》):软件里按 `bpm=142`、每拍 16 格生成,
每步孔距只有 **0.4225mm**(已经很密),机器上根本来不及复位 ——
于是把 **Y 拉伸 150%**,同时把八音盒调到 **260 BPM** 播放,
纸带冲孔段从 5.1m 变成 **7.67m**(加尾部共 7749mm),而音乐本身只有约 3 分半。
**一首 3 分多钟的快曲子,吃掉了近 8 米纸带。**

⇒ **一支完整的曲子(30 秒以上)通常就要 1 米以上**,快曲子被拉伸后动辄三五米。
市面上那种几十厘米的短纸带,打一首完整曲子会"意犹未尽"地断在半路;
而**整卷太短时更尴尬——最后剩下的纸带不够一首曲子**,只能打半首浪费掉。
所以按"整卷能打几首"来买,比按"每卷多少钱"买更划算。

> 想让纸带省一点:把八音盒能承受的**最低 BPM** 用起来(孔距就不必拉那么长)。
> 但**孔距太密音梳会来不及复位、直接漏音**,所以 BPM 的下限由你的音梳决定,不能一味求省;
> 反过来,音梳复位余量越大,能用的 BPM 越高、越省纸。

### 实测好用的廉价平替

先后试过很多种纸带(含塑料带)之后,找到了一款便宜好用的:

![廉价平替纸带(空白无网格,整卷 250 米)](docs/images/tape-cheap.png)

| | 正常纸带(带网格) | 廉价平替(空白) |
|---|---|---|
| 价格 / 长度 | 30 元 / 25 m | **40 元 / 250 m** |
| **每米单价** | 1.20 元/m | **0.16 元/m(便宜约 7.5 倍)** |
| 整卷能打音乐 | 约 26 分钟 | **约 260 分钟** |
| 购买链接 | [淘宝·带网格纸带](https://detail.tmall.com/item.htm?id=561088240482&skuId=5751016689179) | [淘宝·空白纸带](https://detail.tmall.com/item.htm?id=846819175535&skuId=5632133309096) |

> 价格与链接为 2026-09 记录,可能变动;两家都是普通电商店铺,与本项目无利益关系。
> 选型时注意两点:**纸带宽度要够 70mm**(见 [docs/MACHINE.md §2](docs/MACHINE.md)),
> 以及**厚度/挺度要能稳定走带**——太软的纸在送带辊之间会打滑,直接影响孔距精度。
...
docs/images/     文档插图(界面效果图、GRBL 实机设置、弹簧生成/合装、纸带对比)
```
...
- 长曲目(当前工程纸带已达 **3.31m、约 18 分钟**)尚未支持**分段/断点续打**;
  上机前务必确认实际行程 ≥ 纸带总长(软限位关闭时 `$131` 不参与判断,见 [docs/MACHINE.md §7.3](docs/MACHINE.md))。
